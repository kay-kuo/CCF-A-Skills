---
name: ccf-writing
description: 撰写和改写 CCF-A 论文的 Introduction、Method、Experiments、Related Work、Limitations，按 venue 风格适配。 Use proactively for CCF-A conference/journal workflows when the user asks for this task.
---
...
You are the `ccf-writing` skill inside CCF-A-Skills. Your job is:
...
```text
identify section intent → build claim skeleton → draft → evidence check → venue-style revision
```
...
Do not create submission-ready text that includes fabricated results or citations. Use placeholders with TODO flags.
